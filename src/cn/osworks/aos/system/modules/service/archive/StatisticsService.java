...
public class StatisticsService extends JdbcDaoSupport {
	...
	public List<Archive_tablenamePO> findTablename_children() {
		// TODO Auto-generated method stub
		List<Archive_tablenamePO> alist=new ArrayList<Archive_tablenamePO>();
		String sql="select * from archive_TableName_children order by convert(int,indx) asc";
		List<Map<String,Object>> list = jdbcTemplate.queryForList(sql);
		if(list!=null&&list.size()>0){
			for(int i=0;i<list.size();i++){
				Map<String, Object> map = list.get(i);
	...
	public List<Map<String, Object>> countMath(Dto dto) {
		...
		for(int i=0;i<list.size();i++){
			...
			String sql2="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablename+" where nd='"+list.get(i).get("nd")+"' and bgqx='D10' and qzmc='"+qzmc+"'";
			List<Map<String,Object>> list2=jdbcTemplate.queryForList(sql2);
			String sql3="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablename+" where nd='"+list.get(i).get("nd")+"' and bgqx='D30' and qzmc='"+qzmc+"'";
			List<Map<String,Object>> list3=jdbcTemplate.queryForList(sql3);
			String sql4="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablename+" where nd='"+list.get(i).get("nd")+"' and bgqx='Y' and qzmc='"+qzmc+"'";
			List<Map<String,Object>> list4=jdbcTemplate.queryForList(sql4);
			String sql5="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablename+" where nd='"+list.get(i).get("nd")+"' and qzmc='"+qzmc+"'";
