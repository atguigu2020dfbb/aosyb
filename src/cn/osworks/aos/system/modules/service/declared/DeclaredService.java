package cn.osworks.aos.system.modules.service.declared;

import cn.osworks.aos.core.asset.AOSUtils;
import cn.osworks.aos.core.id.AOSId;
import cn.osworks.aos.core.typewrap.Dto;
import cn.osworks.aos.core.typewrap.Dtos;
import cn.osworks.aos.system.dao.mapper.Archive_declaredMapper;
...
@Service
public class DeclaredService extends JdbcDaoSupport {
    @Autowired
    public JdbcTemplate jdbcTemplate;
    @Autowired
    public Archive_declaredMapper archive_declaredMapper;
    @Autowired
     ...
     * @param dto
     */
    public List<Map<String,Object>> listdeclared(Dto dto) {
     ...
     * @param dto
     */
    public boolean adddeclared(Dto dto) {
        String fieldcnnames=dto.getString("fieldcnnames");
        try{
            Archive_declaredPO archive_declaredPO=new Archive_declaredPO();
            AOSUtils.copyProperties(dto,archive_declaredPO);
            archive_declaredPO.setId_(AOSId.uuid());
            archive_declaredPO.setDeclared_list(fieldcnnames);
            archive_declaredMapper.insert(archive_declaredPO);
            return true;
        }catch(Exception e){
            e.printStackTrace();
            return false;
     ...
     * @param out
     */
    public boolean updatedeclared(Dto out) {
        String fieldcnnames=out.getString("fieldcnnames");
        try{
            Archive_declaredPO archive_declaredPO=new Archive_declaredPO();
            AOSUtils.copyProperties(out,archive_declaredPO);
            archive_declaredPO.setDeclared_list(fieldcnnames);
            archive_declaredMapper.updateByKey(archive_declaredPO);
            return true;
        }catch(Exception e){
            e.printStackTrace();
            return false;
    ...
    public boolean deldeclared(Dto outDto) {
        try{
            archive_declaredMapper.deleteByKey(outDto.getString("id_"));
            return true;
        }catch(Exception e){
            e.printStackTrace();
            return false;
    ...
    public Map<String,Object> getdeclared(Dto inDto){
        Map<String,Object> map=new HashMap<String,Object>();
        String sql="select * from archive_deslared where id_='"+inDto.getString("id_")+"'";
        List<Map<String,Object>> list=jdbcTemplate.queryForList(sql);
        if(list!=null&&list.size()>0){
            map=list.get(0);
        }
