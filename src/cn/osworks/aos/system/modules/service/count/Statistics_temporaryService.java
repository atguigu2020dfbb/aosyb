...
@Service
public class Statistics_temporaryService extends JdbcDaoSupport {
	@Autowired
	...
	public List<Map<String, Object>> countMath(Dto dto) {
		...
		String sql="select distinct(qzmc) as qzmc from "+tablenames+" where nd="+nd;
		List<Map<String,Object>> list=jdbcTemplate.queryForList(sql);
		//创建list集合，并存储到集合中
		List<Map<String,Object>> countlist=new ArrayList<Map<String,Object>>();
		for(int i=0;i<list.size();i++){
			//设计sql语句查询10年，30年，永久的各个数量并组成集合
			String sql2="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where nd='"+nd+"' and bgqx='D10' and qzmc='"+list.get(i).get("qzmc")+"'";
			List<Map<String,Object>> list2=jdbcTemplate.queryForList(sql2);
			String sql3="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where nd='"+nd+ "' and bgqx='D30' and qzmc='"+list.get(i).get("qzmc")+"'";
			List<Map<String,Object>> list3=jdbcTemplate.queryForList(sql3);
			String sql4="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where nd='"+nd+"' and bgqx='Y' and qzmc='"+list.get(i).get("qzmc")+"'";
			List<Map<String,Object>> list4=jdbcTemplate.queryForList(sql4);
			String sql5="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where nd='"+nd+"' and qzmc='"+list.get(i).get("qzmc")+"'";
			List<Map<String,Object>> list5=jdbcTemplate.queryForList(sql5);
			Map<String,Object> map=new HashMap<String,Object>();
			map.put("qzmc",list.get(i).get("qzmc"));
			map.put("nd",nd);

			if(list2.get(0).get("sumjs")==null){
				map.put("js_10",0);
			}else{
				map.put("js_10",list2.get(0).get("sumjs"));
			}
			if(list2.get(0).get("sumhs")==null){
				map.put("hs_10",0);
			}else{
				map.put("hs_10",list2.get(0).get("sumhs"));
			}
			if(list2.get(0).get("sumys")==null){
				map.put("ys_10",0);
			}else{
				map.put("ys_10",list2.get(0).get("sumys"));
			}


			if(list3.get(0).get("sumjs")==null){
				map.put("js_30",0);
			}else{
				map.put("js_30",list3.get(0).get("sumjs"));
			}
			if(list3.get(0).get("sumhs")==null){
				map.put("hs_30",0);
			}else{
				map.put("hs_30",list3.get(0).get("sumhs"));
			}
			if(list3.get(0).get("sumys")==null){
				map.put("ys_30",0);
			}else{
				map.put("ys_30",list3.get(0).get("sumys"));
			}


			if(list4.get(0).get("sumjs")==null){
				map.put("js_y",0);
			}else{
				map.put("js_y",list4.get(0).get("sumjs"));
			}
			if(list4.get(0).get("sumhs")==null){
				map.put("hs_y",0);
			}else{
				map.put("hs_y",list4.get(0).get("sumhs"));
			}
			if(list4.get(0).get("sumys")==null){
				map.put("ys_y",0);
			}else{
				map.put("ys_y",list4.get(0).get("sumys"));
			}


			if(list5.get(0).get("sumjs")==null){
				map.put("js_z",0);
			}else{
				map.put("js_z",list5.get(0).get("sumjs"));
			}
			if(list5.get(0).get("sumhs")==null){
				map.put("hs_z",0);
			}else{
				map.put("hs_z",list5.get(0).get("sumhs"));
			}
			if(list5.get(0).get("sumys")==null){
				map.put("ys_z",0);
			}else{
				map.put("ys_z",list5.get(0).get("sumys"));
			}
			countlist.add(map);
