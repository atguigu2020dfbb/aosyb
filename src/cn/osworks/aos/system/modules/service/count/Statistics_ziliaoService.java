...
@Service
public class Statistics_ziliaoService extends JdbcDaoSupport {
	@Autowired
	...
	public List<Map<String, Object>> countMath(Dto dto) {
		...
		String sql="select distinct(nd) as nd from "+tablenames+" where qzh='"+qzmc+"'";
		List<Map<String,Object>> list=jdbcTemplate.queryForList(sql);
		//创建list集合，并存储到集合中
		List<Map<String,Object>> countlist=new ArrayList<Map<String,Object>>();
		/*for(int i=0;i<list.size();i++){
			//设计sql语句查询10年，30年，永久的各个数量并组成集合
			String sql2="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where qzh='"+qzmc+"' and bgqx='D10' and nd='"+list.get(i).get("nd")+"'";
			List<Map<String,Object>> list2=jdbcTemplate.queryForList(sql2);
			String sql3="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where qzh='"+qzmc+ "' and bgqx='D30' and nd='"+list.get(i).get("nd")+"'";
			List<Map<String,Object>> list3=jdbcTemplate.queryForList(sql3);
			String sql4="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where qzh='"+qzmc+"' and bgqx='Y' and nd='"+list.get(i).get("nd")+"'";
			List<Map<String,Object>> list4=jdbcTemplate.queryForList(sql4);
			String sql5="select sum(js) as sumjs,sum(hs) as sumhs,sum(ys) as sumys from "+tablenames+" where qzh='"+qzmc+"' and nd='"+list.get(i).get("nd")+"'";
