...
import cn.osworks.aos.system.dao.mapper.Archive_topicMapper;
...
public class SystemService {
	...
	public List<Aos_sys_modulePO> getBizModulesOfUserG(Dto inDto) {
		inDto.put("grant_type_", DicCons.GRANT_TYPE_BIZ);
		List<Aos_sys_modulePO> listAos_sys_modulePO = getModulesByUserG(inDto);
		listAos_sys_modulePO.addAll(getModulesByPost(inDto));
		listAos_sys_modulePO.addAll(getModulesByRole(inDto));
		// 去除重复值记录
		List<Aos_sys_modulePO> outList = AOSListUtils.distinct(listAos_sys_modulePO, Aos_sys_modulePO.class);
	...
	public List<Aos_sys_modulePO> getBizModulesOfUser4(Dto inDto) {
		inDto.put("grant_type_", DicCons.GRANT_TYPE_BIZ);
		List<Aos_sys_modulePO> listAos_sys_modulePO = getModulesByUser(inDto);
		listAos_sys_modulePO.addAll(getModulesByPost(inDto));
		listAos_sys_modulePO.addAll(getModulesByRole(inDto));
	 ...
	 * @return
	 */
	public List<Aos_sys_modulePO> getModulesByUser2(Dto inDto) {
		inDto.put("fnLength", DBDialectUtils.fnLength(sysDao.getDatabaseId()));
		inDto.remove("_order");
		List<Aos_sys_modulePO> modulesList = sysDao.list("Auth.getModulesByUser2", inDto);
		return modulesList;
	}
	public List<Aos_sys_modulePO> getModulesByUserG(Dto inDto) {
		inDto.put("fnLength", DBDialectUtils.fnLength(sysDao.getDatabaseId()));
		inDto.remove("_order");
		List<Aos_sys_modulePO> modulesList = sysDao.list("Auth.getModulesByUser2", inDto);
		return modulesList;
	...
	public List<Aos_sys_modulePO> getModulesByUser5(Dto inDto) {
		inDto.put("fnLength", DBDialectUtils.fnLength(sysDao.getDatabaseId()));
		inDto.remove("_order");
		List<Aos_sys_modulePO> modulesList = sysDao.list("Auth.getModulesByUser5", inDto);
		return modulesList;
	}
	/**
	 * 根据用户查询通过岗位得到的授权集合
	 ...
	 * @param qDto
	 */
	public List<Dto> listcategory(Dto qDto) {
		qDto.setOrder("sort_no_");
		...
		for (Aos_sys_modulePO aos_sys_modulePO : list) {
			Dto treeNode = Dtos.newDto();
			AOSUtils.copyProperties(aos_sys_modulePO, treeNode);
			treeNode.put("id", aos_sys_modulePO.getId_());
			treeNode.put("text", aos_sys_modulePO.getName_());
			String icon_name_ = aos_sys_modulePO.getIcon_name_();
			if (AOSUtils.isNotEmpty(icon_name_)) {
				treeNode.put("icon", iconPath + icon_name_);
			}
