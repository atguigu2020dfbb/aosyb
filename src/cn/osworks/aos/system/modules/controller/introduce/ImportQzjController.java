...
import cn.osworks.aos.system.modules.service.introduce.ImportQzjService;
 ...
 */
@Controller
@RequestMapping(value ="archive/importqzj")
public class ImportQzjController {
	@Autowired
	private ImportQzjService importService;
	...
	public String loadExcelGrid(HttpServletRequest request,HttpServletResponse response){
		...
		request.setAttribute("tid", qDto.getString("tid"));
	 ...
	 * @param response
	 */
	@RequestMapping(value="listQzjEnnamelist")
	public void listQzjEnnamelist(HttpServletRequest request,HttpServletResponse response){
		Dto qDto = Dtos.newDto(request);
	...
	public void listComboBox(HttpServletRequest request,HttpServletResponse response){
		Dto qDto = Dtos.newDto(request);
		List<Archive_tablefieldlistPO> list = importService.listComboBox2(qDto.getString("tablename"));
		String outString =AOSJson.toGridJson(list);
		WebCxt.write(response, outString);
	}
	 ...
	 * @param response
	 */
	@RequestMapping(value="insertData_qzdw")
	public void insertData_qzdw(HttpServletRequest request,HttpServletResponse response){
		Dto inDto = Dtos.newDto(request);
		Dto outDto = importService.InsertDb_qzdw(inDto);
		String outString = AOSJson.toJson(outDto);
		WebCxt.write(response, outString);
	}
	/**
