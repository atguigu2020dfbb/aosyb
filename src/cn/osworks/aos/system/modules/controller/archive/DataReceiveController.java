...
import cn.osworks.aos.system.modules.service.archive.DataReceiveService;
 ...
 */
@Controller
@RequestMapping(value = "archive/datareceive")
public class DataReceiveController {
	@Autowired
	private DataReceiveService dataService;
	@Autowired
	...
	public String initData(HttpSession session, HttpServletRequest request,
		...
		//String nd=dataService.findParentNd(qDto);
		...
		if(associations_==""||associations_==null||associations_.equals("非关联显示")){
			...
			return "aos/receive/datareceive.jsp";
	 ...
	 * @return
	 */
	@RequestMapping(value = "initreceiveData")
	public String initreceiveData(HttpSession session, HttpServletRequest request,
						   HttpServletResponse response) {
		Dto qDto = Dtos.newDto(request);
		List<Archive_tablefieldlistPO> titleDtos = dataService
				.getDataFieldListTitle(qDto.getString("tablename"));
		...
		String treename=dataService.findTree(qDto);
		String nd=dataService.findParentNd(qDto);
		String queryclass = dataService.isAll(qDto);
		request.setAttribute("cascode_id_", qDto.getString("cascode_id_"));
		request.setAttribute("tablename", qDto.getString("tablename"));
	...
	public void listreceive(HttpServletRequest request, HttpServletResponse response){
		...
		List<Map<String, Object>> fieldDtos=dataService.listreceive();
	...
	public void listAccounts2(HttpServletRequest request,
		...
		if(tablename==""||tablename==null||tablename=="null"){
			...
			UserInfoVO userInfoVO = (UserInfoVO) session
					.getAttribute("_userInfoVO");
			List<Dto> fieldDtos = dataService.getDataFieldListDisplayAll3(inDto,
					userInfoVO.getAccount_());
			int pCount = dataService.getPageTotal(inDto);
			String outString = AOSJson.toGridJson(fieldDtos, pCount);
			// 将分类坐标存放到session中
    ...
    public void deletePrint(HttpServletRequest request,
        ...
        logMapper.insert(logPO);
        String json=AOSJson.toJson(outDto);
        WebCxt.write(response, json);
    }
    /**
     *
     * 彻底删除条目
	...
	//转正式库页面进入
	@RequestMapping("initReceiveToManageData")
	public String initReceiveToManageData(HttpServletRequest request, HttpServletResponse response){
		return "aos/receive/datatemporaryToManage.jsp";
	}
    /**
     * 页面初始化
     *
    ...
    public String initInput(HttpServletRequest request,HttpServletResponse response,HttpSession session) throws UnsupportedEncodingException{
        ...
        if(listtablename==null||listtablename.equals("")||listtablename.equals("null")){
            return "aos/receive/datatemporaryToManage.jsp?time="+new Date().getTime();
        }
        List<Archive_tablefieldlistPO> title = dataService.getDataFieldListTitle(listtablename);
        request.setAttribute("listtablename", listtablename);
        request.setAttribute("tablenamedesc", tablenamedesc);
        request.setAttribute("fieldDtos", title);
        return "aos/receive/datatemporaryToManage.jsp?time="+new Date().getTime();
    }
    @RequestMapping("listReceiveTablename")
    public void listReceiveTablename(HttpServletRequest request,HttpServletResponse response,HttpSession session){
        List<Archive_tablenamePO> list = dataService.findReceiveTablename();
        String outString =AOSJson.toGridJson(list);
        WebCxt.write(response, outString);
    }
	@RequestMapping("listReceivetoManage")
    public void listReceivetoManage(HttpServletRequest request,HttpServletResponse response,HttpSession session)throws Exception{
		Dto dto=Dtos.newDto(request);
		List<Map<String,Object>> list =dataService.listReceivetoManage(dto);
		String outString =AOSJson.toGridJson(list);
		WebCxt.write(response, outString);
    }
	@RequestMapping("updatemanage")
    public void updatemanage(HttpServletRequest request,HttpServletResponse response,HttpSession session){
		Dto inDto = Dtos.newDto(request);
		Dto out =Dtos.newDto();
		boolean b=dataService.updatemanage(inDto);
		if(b){
			out.setAppMsg("提交完成!");
		}else{
			out.setAppMsg("提交失败!");
	...
	@RequestMapping(value = "updatemanageyes")
	public void updatemanageyes(HttpServletRequest request,
								 HttpServletResponse response, HttpSession session){
		Dto inDto = Dtos.newDto(request);
		Dto out =Dtos.newDto();
		UserInfoVO userInfoVO = WebCxt.getUserInfo(session);
		boolean b=dataService.updatemanageyes(inDto,userInfoVO);
		if(b){
			out.setAppMsg("操作完成!");
		}else{
			out.setAppMsg("操作失败!");
	...
	@RequestMapping(value = "updatemanageno")
	public void updatemanageno(HttpServletRequest request,
								  HttpServletResponse response, HttpSession session){
		Dto inDto = Dtos.newDto(request);
		Dto out =Dtos.newDto();
		UserInfoVO userInfoVO = WebCxt.getUserInfo(session);
		boolean b=dataService.updatemanageno(inDto,userInfoVO);
		if(b){
			out.setAppMsg("操作完成!");
		}else{
			out.setAppMsg("操作失败!");
	...
	@RequestMapping(value = "receiveTomanageData")
	public void receiveTomanageData(HttpServletRequest request,
									   HttpServletResponse response, HttpSession session)throws Exception{
		Dto inDto = Dtos.newDto(request);
		UserInfoVO userInfoVO = WebCxt.getUserInfo(session);
		Dto out=dataService.receiveTomanageData(inDto,userInfoVO);
	...
	public void getQzh(HttpServletRequest request,HttpServletResponse response,HttpSession session){
        Dto outDto = Dtos.newDto(request);
        Dto out=Dtos.newDto();
        out.put("qzh",outDto.getString("qzh"));
        String json=AOSJson.toJson(out);
        WebCxt.write(response, json);
    }
}