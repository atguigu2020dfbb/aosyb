package cn.osworks.aos.system.modules.controller.census;
...
@Controller
@RequestMapping(value="census/census")
public class CensusController {
    ...
    public String initData(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws UnsupportedEncodingException {
        ...
        return "aos/census/census.jsp?time="+new Date().getTime();
    }
    /**
     * 全宗列表
     *
     ...
     * @throws UnsupportedEncodingException
     */
    @RequestMapping("listcensus")
    public void listcensus(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws UnsupportedEncodingException{
        Dto dto=Dtos.newDto();
        List<Map<String,Object>>  list=censusService.listgenerals(dto);
    ...
    public void addcensus(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws UnsupportedEncodingException{
        Dto dto=Dtos.newDto(request);
        Dto outDto=Dtos.newDto();
        boolean b=censusService.addcensus(dto);
        if(b){
            outDto.setAppCode(1);
        }else{
            outDto.setAppCode(-1);
     ...
     * @param session
     */
    @RequestMapping(value="delcensus")
    public void delcensus(HttpServletRequest request, HttpServletResponse response, HttpSession session){
        Dto out=Dtos.newDto(request);
        Dto outDto=Dtos.newDto();
        boolean b=censusService.delcensus(out);
        if(b){
            outDto.setAppCode(1);
        }else{
            outDto.setAppCode(-1);
     ...
     * @param session
     */
    @RequestMapping(value="updatecensus")
    public void updatecensus(HttpServletRequest request, HttpServletResponse response, HttpSession session){
        Dto out=Dtos.newDto(request);
        Dto outDto=Dtos.newDto();
        boolean b=censusService.updatecensus(out);
        if(b){
            outDto.setAppCode(1);
        }else{
            outDto.setAppCode(-1);
    ...
    public void getQueryTitle(HttpServletRequest request,HttpServletResponse response,HttpSession session){
        ...
        List<Archive_tablefieldlistPO> list=censusService.getQueryTitle(inDto);
    ...
    public void gettablename(HttpServletRequest request,HttpServletResponse response,HttpSession session){
        ...
        Dto inDto = Dtos.newDto(request);
        String tablename=censusService.gettablename(inDto);
        Dto out=Dtos.newDto();
    ...
    public void getQueryTitlename(HttpServletRequest request,HttpServletResponse response,HttpSession session){
        ...
        Dto inDto = Dtos.newDto(request);
        List<Archive_tablefieldlistPO> list=censusService.getQueryTitlename(inDto);
        WebCxt.write(response, AOSJson.toGridJson(list));
    }
}
