...
<aos:onready>
	<aos:viewport layout="border">
		<aos:gridpanel id="_g_collect" url="listcollect.jhtml" region="center" split="true"
			onrender="_g_entity_query" hidePagebar="true"
			 enableLocking="false" >
			<aos:docked>
				<aos:dockeditem xtype="tbtext" text="实体信息" />
				<aos:dockeditem text="新建征集" icon="add2.png"  onclick="_g_add_pici" id="_f_add_pici"/>
			...
			<aos:column dataIndex="zjmc" header="征集名称" />
			<aos:column dataIndex="zjms" header="征集描述" />
			<aos:column dataIndex="zjsj" header="征集时间"  />
			<aos:column dataIndex="zjr" header="征集人"  />
	...
	<aos:window id="_w_pici" title="新增任务" >
		<aos:formpanel id="_f_pici" width="420" layout="anchor" labelWidth="90">
			<aos:textfield name="zjmc" fieldLabel="征集名称"  />
			<aos:datefield name="zjsj" fieldLabel="征集时间" format="Y-m-d"
						   columnWidth="1.0" />
			<aos:textfield name="zjr" fieldLabel="征集人"  />
			<aos:textareafield name="zjms" fieldLabel="征集描述"  />
		</aos:formpanel>
		<aos:docked dock="bottom" ui="footer">
			<aos:dockeditem xtype="tbfill" />
	...
	function _w_print_load(){
		...
		function _f_pici_submit(){
			AOS.ajax({
				url: 'addcollect.jhtml',
				forms:_f_pici,
				ok: function (data) {
					if(data.appcode===1){
						AOS.tip("添加成功!");
						_w_pici.hide();
						_g_collect_store.reload();
        ...
        function _g_receive_pici(){
            var selection = AOS.selection(_g_collect, 'id_');
            if (AOS.empty(selection)) {
                AOS.tip('提交前请先选中数据。');
                return;
            }
            var record = AOS.selectone(_g_collect);
            var id_="";
            var flag_submit="";
            if(!AOS.empty(record)){
                id_=record.data.id_;
            ...
            var msg = AOS.merge('确认要提交选中的[{0}]个任务数据吗？', AOS.rows(_g_collect));
            AOS.confirm(msg, function (btn) {
                if (btn === 'cancel') {
                    AOS.tip('提交操作被取消。');
                    return;
        ...
        function _w_formal_receive(){
            ...
            if(!AOS.empty(record)){
                ...
                AOS.ajax({
                    params:{id_:id_},
                    url : 'collectToreceiveData.jhtml',
                    ok : function(data) {
                        AOS.tip(data.appmsg);
						_g_collect_store.reload();
                    }
