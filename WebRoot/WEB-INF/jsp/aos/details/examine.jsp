...
<aos:html>
	...
	<aos:onready>
		<aos:viewport layout="fit">
			<aos:gridpanel id="_g_data" region="center" url="listtrial.jhtml" onrender="_w_next_trial_show"
						   autoScroll="true" pageSize="20" enableLocking="true" >
				<aos:docked forceBoder="0 0 1 0">
					<aos:dockeditem xtype="tbtext" text="鉴定审核" />
					...
					<aos:dockeditem text="再审操作" icon="c_key.png" id="next_operator"
									onclick="_check_next" />
					<aos:dockeditem text="终审操作" icon="c_key.png" id="last_operator"
									onclick="_check_last" />
				</aos:docked>
			...
			<aos:window id="_w_first_trial" title="编研列表" autoScroll="true" height="500" width="800" onshow="_w_first_trial_show">
				<aos:gridpanel id="_g_first_trial" region="east" url="listfirsttrial.jhtml"
					...
					<aos:docked>
						<aos:dockeditem xtype="tbtext" text="列表" />
						<aos:dockeditem onclick="first_trial_examine_show" text="合稿操作" icon="ok.png" />
					</aos:docked>
					<aos:column type="rowno" />
			...
			<aos:window id="_w_next_trial" title="再审列表" autoScroll="true" height="500" width="800" onshow="_w_next_trial_show">
				<aos:gridpanel id="_g_next_trial" region="east" url="listnexttrial.jhtml"
					...
					<aos:docked>
						<aos:dockeditem xtype="tbtext" text="列表" />
						<aos:dockeditem onclick="next_trial_checkup_" text="审核" icon="more/tools-check-spelling.png" />
					</aos:docked>
					<aos:column type="rowno" />
					<aos:column header="流水号" dataIndex="id_" hidden="true" />
					<aos:column  dataIndex="write_flag" hidden="true" />
			...
			<aos:window id="_w_last_trial" title="终审列表" autoScroll="true" height="500" width="800" onshow="_w_last_trial_show">
				<aos:gridpanel id="_g_last_trial" region="east" url="listlasttrial.jhtml"
					...
					<aos:docked>
						<aos:dockeditem xtype="tbtext" text="列表" />
						<aos:dockeditem onclick="last_trial_examine_show" text="操作" icon="more/tools-check-spelling.png" />
					</aos:docked>
					<aos:column type="rowno" />
					<aos:column header="流水号" dataIndex="id_" hidden="true" />
					<aos:column  dataIndex="write_flag" hidden="true" />
			...
			<aos:window id="_next_list_show" title="详情">
				<aos:formpanel id="_g_next_list_show" width="500" layout="anchor"
							   labelWidth="90">
					<aos:hiddenfield name="id_" />
					...
					<aos:textareafield name="last_message" fieldLabel="终审人描述"  columnWidth="1.0"  readOnly="true"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem onclick="appraisal_archive" text="鉴定" icon="more/identity-2.png" />
					<aos:dockeditem onclick="#_next_list_show.hide();" text="关闭" icon="close.png" />
				</aos:docked>
			</aos:window>
...
// 		        Ext.getCmp("_g_destroy").getSelectionModel().getSelection()[0].data.id_;
			...
			function _check_next(){
				...
				if (AOS.empty(selection)) {
					...
					return;
						...
						if (record) {
							_next_list_show.show();
							_g_next_list_show.loadRecord(record);
							AOS.ajax({
								...
								ok: function (data) {
									if(data.appmsg===""){
										...
										_g_next_list_show.getForm().findField('last_message').setValue(data.appmsg);
									}
								}
							});
						}
				...
				var checkup_type=AOS.selectone(_g_data).raw.checkup_type;
				var first_compilationperson=AOS.selectone(_g_data).raw.first_compilationperson;
				//得到界面选中的档案的列表
				if (AOS.empty(selection)) {
					AOS.tip('请选择要操作的方案!');
					return;
					...
					//弹出档案鉴定窗口
					window.parent.fnaddtab('','档案详情','/archive/checkup/next_details_all.jhtml?id_='+id_+'&checkup_type='+checkup_type+'&tablename='+tablename.getValue());
				}*/
			...
			function _check_last(){
				...
				if (AOS.empty(selection)) {
					...
					//弹出档案鉴定窗口
					window.parent.fnaddtab('','档案详情','/archive/checkup/last_details_all.jhtml?id_='+id_+'&checkup_type='+checkup_type+'&tablename='+tablename.getValue());
				}
			}
			...
			function next_trial_examine_show(){
				...
				var checkup_type=AOS.selectone(_g_next_trial).raw.checkup_type;
				var first_compilationperson=AOS.selectone(_g_next_trial).raw.first_compilationperson;
				//得到界面选中的档案的列表
				if (AOS.empty(selection)) {
					AOS.tip('请选择要操作的方案!');
					return;
			...
			function last_trial_examine_show(){
				...
				var checkup_type=AOS.selectone(_g_last_trial).raw.checkup_type;
				var first_compilationperson=AOS.selectone(_g_last_trial).raw.first_compilationperson;
				//得到界面选中的档案的列表
				if (AOS.empty(selection)) {
					AOS.tip('请选择要操作的方案!');
					return;
			...
			function appraisal_archive(){
				...
				var id_=AOS.selectone(_g_data).raw.id_;
				var checkup_type=AOS.selectone(_g_data).raw.checkup_type;
				var first_compilationperson=AOS.selectone(_g_data).raw.next_compilationperson;
				//得到界面选中的档案的列表
				...
				if (AOS.empty(selection)) {
					...
					window.parent.fnaddtab('','档案详情','/archive/checkup/next_archive_details.jhtml?id_='+id_+'&checkup_type='+checkup_type+'&tablename='+tablename.getValue());
				}
			}
		</script>
	</aos:onready>
