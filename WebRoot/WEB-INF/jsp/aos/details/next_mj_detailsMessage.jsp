	...
	<aos:onready>
		<aos:viewport layout="border">
			<aos:gridpanel id="_g_account" url="listAccounts2.jhtml" region="north" split="true" pageSize="${pagesize }"
				...
				<aos:menu>
					<aos:menuitem text="绝密-机密" onclick="!save_mj"  />
					<aos:menuitem text="机密-秘密" onclick="!save_mj"  />
					<aos:menuitem text="绝密-秘密" onclick="!save_mj"  />
					<aos:menuitem text="秘密-绝密" onclick="!save_mj"  />
					<aos:menuitem text="机密-绝密" onclick="!save_mj"  />
					<aos:menuitem text="秘密-机密" onclick="!save_mj"  />
				</aos:menu>
				<aos:column dataIndex="id_" header="流水号" hidden="true" />
				<aos:column dataIndex="_mj" header="密级类型" hidden="true" />
			...
			<aos:panel region="east" id="edit_message"  split="true" >
				<aos:formpanel id="_f_description" title="鉴定" layout="column" >
					...
					<aos:radioboxgroup fieldLabel="密级类型"  id="mjleixing" columns="[100,120,100]">
						<aos:radiobox name="_mj" boxLabel="绝密-机密" />
						<aos:radiobox name="_mj" boxLabel="机密-秘密" />
						<aos:radiobox name="_mj" boxLabel="绝密-秘密" />
						<aos:radiobox name="_mj" boxLabel="秘密-绝密" />
						<aos:radiobox name="_mj" boxLabel="机密-绝密" />
						<aos:radiobox name="_mj" boxLabel="秘密-机密" />
					</aos:radioboxgroup>
				...
				<aos:docked dock="bottom" ui="footer">
					...
					<aos:dockeditem onclick="_next_save_mj_details" text="提交终审" icon="ok.png" />
				</aos:docked>
			</aos:panel>
			...
			<aos:window id="_w_next_mj_message" title="再审密级鉴定说明" width="500" >
				<aos:formpanel  id="_f_next_mj_message" width="480">
					<aos:textareafield fieldLabel="操作说明" id="next_mj_message"  name="next_mj_message" width="450" allowBlank="false"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem text="同意" onclick="_save_next_mj_message" icon="agree.png" />
					<aos:dockeditem text="拒绝" onclick="no_save_next_mj_message" icon="against.png" />
					<aos:dockeditem onclick="#_w_next_mj_message.hide();" text="关闭"
									icon="close.png" />
				</aos:docked>
			</aos:window>
		</aos:viewport>
		<script type="text/javascript">
			...
			function nextselect(){
				if(typeof(AOS.selectone(_g_account))!='undefined'){
					...
					if(Number(index)+1>=_g_account_store.getCount()){
						...
						//字段信息传递过去
						_w_data_u_onshow(_g_account_store.getAt(index+1).get("id_"),_g_account_store.getAt(index+1).get("tablename"));
						//var id_=_g_account_store.getAt(index+1).get("id_");
						//此时坐标加一加载图片
						//path();
			...
			function radio_check(){
				var mj=AOS.selectone(_g_account).raw._mj;
				var equRad=Ext.getCmp("mjleixing").getRefItems();
				for(var b=0;b<equRad.length;b++){
					if(equRad[b].boxLabel===mj){
...
//把这个值存放到数据库中
			...
			//保存开放信息
			function _next_save_mj_details(){
				//此时把当前的信息全部保存到数据表中并添加信息
				_w_next_mj_message.show();
				Ext.getCmp("next_mj_message").setValue("鉴定复审已完毕，请终审。");
			}
			function _save_next_mj_message(){
				//执行保存操作
				...
				AOS.ajax({
					url: 'savenextmjdetails.jhtml',
					forms:_f_next_mj_message,
					params:{
			...
			function no_save_next_mj_message(){
				//执行保存操作
				//var tablename_gridData = JSON.stringify(Ext.pluck(_g_tablename_details_store.data.items, 'data'));
				var user="<%=session.getAttribute("user")%>";
				var _classtree=Ext.getCmp("_classtree").getValue();
