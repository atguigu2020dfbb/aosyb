...
<aos:onready>
	<aos:viewport layout="border">
		...
		<aos:panel region="east" id="edit_message"  split="true" autoScroll="true">
			<aos:formpanel id="_f_description" title="鉴定"  layout="column"  autoScroll="true" width="600">
				...
				<aos:radioboxgroup fieldLabel="初鉴结果"  id="kaifangleixing" columnWidth="0.98" columns="[100,120,100]">
			...
			<aos:docked dock="bottom" ui="footer">
				...
				<aos:dockeditem onclick="_first_save_kf_details" text="提交复审" icon="ok.png" />
			</aos:docked>
		</aos:panel>
		...
		<aos:window id="_w_first_kf_message" title="初鉴开放鉴定说明" width="500" >
			<aos:formpanel  id="_f_first_kf_message" width="480">
				<aos:textareafield fieldLabel="操作说明" id="first_kf_message" name="first_kf_message" width="450" allowBlank="false"/>
			</aos:formpanel>
			<aos:docked dock="bottom" ui="footer">
				<aos:dockeditem xtype="tbfill" />
	...
	<script type="text/javascript">
			...
			AOS.ajax({
				...
				ok: function (data) {
					...
					for(var i = 0; i < xqCheck.length; i++){
						if(data.sfdjcl!=null&&data.sfdjcl!=""&&data.sfdjcl.indexOf(xqCheck.get(i).boxLabel)!=-1){
		...
		//电子文件加载方法
		function path(grid, rowIndex, columnIndex, e) {
		...
		function radio_check(){
			...
			var sfCheck = Ext.getCmp('kaifangleixing').items;
			for(var i = 0; i < sfCheck.length; i++){
				if(data.sfkf==sfCheck.get(i).boxLabel){
					sfCheck.get(i).setValue(true);
				}
			}
			_f_description.form.findField("sfkf").setValue(data.sfkf);
...
//把这个值存放到数据库中
		...
		function _first_save_kf_details(){
			...
			Ext.getCmp("first_kf_message").setValue("鉴定初审已完毕，请复审。");
		}
		function _save_first_kf_message(){
			...
			AOS.ajax({
				url: 'savefirstkfdetails.jhtml',
				forms:_f_first_kf_message,
				params:{
