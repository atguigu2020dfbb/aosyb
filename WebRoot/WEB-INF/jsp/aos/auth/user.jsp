...
<aos:onready>
	<aos:viewport layout="border">
		...
		<aos:gridpanel id="_g_user" region="center" onrender="_g_user_query" bodyBorder="1 0 1 0" url="listUsers.jhtml"
			...
			<aos:docked forceBoder="0 0 1 0">
				...
				<aos:dockeditem text="按钮权限" tooltip="按钮权限" onclick="_w_power_show" icon="config1.png" />
			...
			<aos:menu>
				<aos:menuitem text="权限预览" onclick="_w_preview_show" icon="query.png" />
				<aos:menuitem xtype="menuseparator" />
				<aos:menuitem text="普通用户" onclick="_g_putong_person" icon="folder1.png" />
				<aos:menuitem xtype="menuseparator" />
				<aos:menuitem text="用户菜单授权" onclick="_w_user_module_show" icon="icon75.png" />
				<aos:menuitem text="用户岗位授权" onclick="_w_user_post_show" icon="icon137.png" />
				<aos:menuitem text="用户角色授权" onclick="_w_user_role_show" icon="icon134.png" />
			...
			<aos:column header="接收权限" dataIndex="jieshou_module" />
			<aos:column header="鉴定权限" dataIndex="jianding_module" />
			<aos:column header="编研权限" dataIndex="bianyan_module" />
		</aos:gridpanel>
	</aos:viewport>
	...
	<aos:window id="_w_power_button" title="按钮权限" onshow="_get_button_power">
		<aos:formpanel id="_f_power_button" layout="column" width="700" >

			<aos:checkboxgroup fieldLabel="接收模块" id="jieshou_module" columns="[100,100]" columnWidth="0.98">
				<aos:checkbox name="r2" boxLabel="创建人"   />
				<aos:checkbox name="r2" boxLabel="提交申请"   />
				<aos:checkbox name="r2" boxLabel="领导审核" />
			</aos:checkboxgroup>
			<aos:checkboxgroup fieldLabel="鉴定模块" id="jianding_module" columns="[100,100,100,100]" columnWidth="0.98">
				<aos:checkbox name="r3" boxLabel="创建人"  />
				<aos:checkbox name="r3" boxLabel="初鉴" />
				<aos:checkbox name="r3" boxLabel="复核" />
				<aos:checkbox name="r3" boxLabel="终审" />
			</aos:checkboxgroup>
			<aos:checkboxgroup fieldLabel="编研模块" id="bianyan_module" columns="[100,100,100,100,100,100]" columnWidth="0.98">
				<aos:checkbox name="r4" boxLabel="创建人"  />
				<aos:checkbox name="r4" boxLabel="审核" />
				<aos:checkbox name="r4" boxLabel="撰稿" />
				<aos:checkbox name="r4" boxLabel="合稿" />
				<aos:checkbox name="r4" boxLabel="校对" />
				<aos:checkbox name="r4" boxLabel="总编辑" />
			</aos:checkboxgroup>
			<aos:docked dock="bottom">
				<aos:dockeditem xtype="tbfill"/>
				<aos:dockeditem text="添加" icon="add.png" onclick="_f_power_button_save"/>
				<aos:dockeditem text="关闭" icon="close.png" onclick="#_w_power_button.hide()"/>
			</aos:docked>
		</aos:formpanel>
	</aos:window>
	...
	<script type="text/javascript">
			...
			function _g_shuzi_person(){
				var rows = AOS.rows(_g_user);
				if (rows === 0) {
					AOS.tip('请先选择要操作的人员。');
					return;
				}
				var selection = AOS.selectone(_g_user);
				AOS.ajax({
					params: {id_:selection.data.id_,
						compilation_flag_:"数字科员"},
					url: 'updateUser_compilation.jhtml',
					ok: function (data) {
						if (data.appcode === -1) {
							AOS.err(data.appmsg);
						} else {
			...
			//用户按钮权限
			function _w_power_show() {
				var rows = AOS.rows(_g_user);
				if (rows === 0) {
					AOS.tip('请先选择要操作的用户数据。');
					return;
				} else if (rows > 1) {
					AOS.tip('只能对单个用户进行按钮授权操作。');
					return;
				}
				_w_power_button.show();
			}
			...
			function _get_button_power(){
            	AOS.reset(_f_power_button);
				var record = AOS.selectone(_g_user);
				var jieshou = Ext.getCmp('jieshou_module').items;
				for(var i = 0; i < jieshou.length; i++){
					if(record.data.jieshou_module.indexOf(jieshou.get(i).boxLabel)!=-1){
						jieshou.items[i].setValue(true);
					}
				}
				var jianding= Ext.getCmp('jianding_module').items;
				for(var i = 0; i < jianding.length; i++){
					if(record.data.jianding_module.indexOf(jianding.get(i).boxLabel)!=-1){
						jianding.items[i].setValue(true);
					}
				}
				var bianyan= Ext.getCmp('bianyan_module').items;
				for(var i = 0; i < bianyan.length; i++){
					if(record.data.bianyan_module.indexOf(bianyan.get(i).boxLabel)!=-1){
						bianyan.items[i].setValue(true);
					}
				}
			}
			...
			function getUpdateRadio(){
				var jieshou_module = Ext.getCmp('jieshou_module').items;
				var jieshou="";
				for (var i=0; i<jieshou_module.length;i++){
					if(jieshou_module.get(i).checked==true){
						if(jieshou==""){
							jieshou=jieshou_module.get(i).boxLabel;
						}else{
							jieshou=jieshou+","+jieshou_module.get(i).boxLabel;
						}
					}
				}
				var jianding_module = Ext.getCmp('jianding_module').items;
				var jianding="";
				for (var i=0; i<jianding_module.length;i++){
					if(jianding_module.get(i).checked==true){
						if(jianding==""){
							jianding=jianding_module.get(i).boxLabel;
						}else{
							jianding=jianding+","+jianding_module.get(i).boxLabel;
						}
					}
				}
				var bianyan_module = Ext.getCmp('bianyan_module').items;
				var bianyan="";
				for (var i=0; i<bianyan_module.length;i++){
					if(bianyan_module.get(i).checked==true){
						if(bianyan==""){
							bianyan=bianyan_module.get(i).boxLabel;
						}else{
							bianyan=jianding+","+bianyan_module.get(i).boxLabel;
						}
					}
				}
				//得到当亲id和用户
				...
				var account_=record.data.account_;
				var params={
					id_:id_,
					account_:account_,
					jieshou_module:jieshou,
					jianding_module:jianding,
					bianyan_module:bianyan
				};
				return params;
			}
			function _f_power_button_save(){
					var params=getUpdateRadio();
					AOS.ajax({
						params:params,
						url: 'updateUserButton.jhtml',
						ok:function(data){
							if(data.appcode===1){
								_w_power_button.hide();
								_g_user_store.reload();
								AOS.tip(data.appmsg);
							}else {
								AOS.err(AOS.appmsg);
