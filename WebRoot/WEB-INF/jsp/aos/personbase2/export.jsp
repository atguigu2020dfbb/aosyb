...
<aos:onready>
	<aos:viewport layout="border">
		<aos:gridpanel id="_g_data" url="listAccounts_data.jhtml" region="center" split="true"
			...
			<aos:docked>
				...
				<aos:dockeditem text="查询" icon="query.png" id="select_query" onclick="_w_query_show" />
			...
			<aos:plugins>
				<aos:editing id="id_plugin"   onedit="fn_edit"  autoCancel="false"  onbeforeedit="fn_beforeedit"/>
			</aos:plugins>
	...
	<aos:window id="_w_count" title="统计" width="720" autoScroll="true"
		layout="fit">
			<aos:tabpanel id="_tabpanel4" region="center" activeTab="0"
				bodyBorder="0 0 0 0" tabBarHeight="30">
				<aos:tab title="统计" id="_tab_org2">
				<aos:formpanel id="_f_count" width="500" layout="column">
				<aos:hiddenfield name="_classtree" id="_classtree"
				value="${cascode_id_}" />
					<aos:combobox fieldLabel="统计字段" name="countfield" labelWidth="70"
						columnWidth="0.49">
						<aos:option value="_path" display="电子文件" />
						<aos:option value="ys" display="页数" />
					</aos:combobox>
					<aos:combobox fieldLabel="统计方法" name="countmethod" labelWidth="70"
						columnWidth="0.49">
						<aos:option value="max" display="最大值" />
						<aos:option value="count" display="总条数" />
						<aos:option value="avg" display="平均数" />
						<aos:option value="sum" display="总数" />
						<aos:option value="qzh" display="全宗数" />
						<aos:option value="ajh" display="案卷数" />
					</aos:combobox>
					<aos:displayfield name="resultsum" columnWidth="0.99"/>
	...
	<script type="text/javascript">
		...
		function _w_tablename_init(){
			...
			window.location.href="export_initData.jhtml?listtablename="+listtablename+"&tablenamedesc="+tablenamedesc;
		}
		//生成录入界面
	...
	if(emptyfiledcnname==filedcnname&&filedcnname!=null&&filedcnname!=""){
	 ...
	 params["tablename"] =Ext.getCmp("tablename").getValue();
		params["listtablename"] =Ext.getCmp("tablename").getValue();
	_g_data_store.getProxy().extraParams = params;
	_g_data_store.load();
	_w_query_q.hide();
	}
