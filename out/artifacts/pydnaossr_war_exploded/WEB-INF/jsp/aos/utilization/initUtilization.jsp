...
<aos:html>
        ...
        <aos:viewport layout="fit">
            <aos:gridpanel id="_g_utilization" url="listUtilization.jhtml" onrender="_g_utilization_onrender">
                <aos:docked>
                    ...
                    <aos:dockeditem text="新增任务" id="add" icon="add.png" onclick="_w_utilization_show"/>
                    <aos:dockeditem text="修改" icon="edit.png" onclick="_w_utilization_u_show"/>
                    <aos:dockeditem text="删除" icon="del.png" onclick="_g_utilization_del"/>
                    <aos:dockeditem text="查阅" icon="icon71.png" onclick="_g_utilization_read"/>
                    <aos:dockeditem text="登记详情" icon="icon66.png" onclick="_w_utilization_details"/>
                    <aos:dockeditem text="实体调档" icon="icon32.gif" onclick="_w_utilization_dd_show"/>
                    <aos:dockeditem text="已归还" icon="icon35.gif" onclick="_w_utilization_gh"/>
                    <aos:dockeditem text="问题标识" icon="more/page-white_error.png" onclick="_w_utilization_error"/>
                    <aos:dockeditem text="标识列表" icon="more/building-error.png" onclick="_w_utilization_error_list"/>
                </aos:docked>
                <aos:column header="id_" dataIndex="id_"  locked="true"/>
                <aos:column header="任务编号" dataIndex="djbh"/>
                ...
                <aos:column header="审核批注" dataIndex="sppz" />
                <aos:column header="审核人" dataIndex="spr"/>
                <aos:column header="审核时间" dataIndex="spsj"/>
    ...
    <script type="text/javascript">
        ...
        function _g_utilization_del(){
            ...
            AOS.confirm(msg,function(btn){
                ...
                AOS.ajax({
                    ...
                    params:{
                        aos_rows_: AOS.selection(_g_utilization,'id_')
                    },
                    ok:function(data){
                        AOS.tip(data.appmsg);
                        _g_utilization_store.reload();
        ...
        function _w_satisfied_u_show(){
            ...
            if(record){
                _w_satisfied_u.show();
                _f_satisfied_u.loadRecord(record)
            }
        }
        //问题标示
        function _w_utilization_error(){
                var record = AOS.selectone(_g_utilization);
            if(AOS.empty(record)){
                AOS.tip("请选择需要标识的问题!");
                return;
            }
            parent.fnaddtab('','登记详情','/utilization/loadData_error.jhtml?id_='+record.data.id_+'&djbh='+record.data.djbh);
                //parent.fnaddtab('','登记详情','/utilization/initData.jhtml?formid='+record.data.id_)
        }
        function _w_utilization_error_list(){
            parent.fnaddtab('','标记列表','/utilization/list_error.jhtml');
        }
    </script>
    </aos:onready>
</aos:html>