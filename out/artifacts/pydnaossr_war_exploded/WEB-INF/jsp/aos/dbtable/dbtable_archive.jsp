	...
	<aos:onready>
		<aos:viewport layout="border">
			<aos:gridpanel id="_g_table" region="center" url="listAccounts.jhtml" width="450"
				...
				<aos:docked>
					<aos:dockeditem xtype="tbtext" text="档案门类操作" />
				...
				<aos:column header="档案门类名称" dataIndex="tablename" width="90" />
				<aos:column header="档案门类描述" dataIndex="tabledesc" width="80" />
			</aos:gridpanel>
			<aos:gridpanel id="_g_field"  url="listFieldInfos.jhtml" region="east" width="600"
				...
				<aos:docked>
					<aos:dockeditem xtype="tbtext" text="档案门类字段操作" />
					<aos:dockeditem xtype="tbfill" />
			...
			<aos:gridpanel id="_g_correlation" region="east" width="450"
				...
				<aos:docked>
					<aos:dockeditem xtype="tbtext" text="档案门类之间关联" />
					<aos:dockeditem xtype="tbfill" />
			...
			<aos:window id="_w_table_u" title="修改档案门类">
				<aos:formpanel id="_f_table_u" width="800" layout="column"
							   labelWidth="75" height="500">
					<aos:fieldset title="创建档案门类" columnWidth="1" checkboxToggle="false"
								  collapsed="false">
						<aos:hiddenfield name="id_" />
						<aos:textfield fieldLabel="档案门类名称" readOnly="true" name="tablename"
									   vtype="alphanum" emptyText="只能输入字母和数字" columnWidth="0.5" />
						<aos:textfield fieldLabel="档案门类描述" name="tabledesc"
									   columnWidth="0.49" />
					</aos:fieldset>
			...
			<aos:window id="_w_field" title="新增档案门类字段">
				<aos:formpanel id="_f_field" width="500" layout="anchor"
							   labelWidth="90">
					<aos:hiddenfield name="tid" fieldLabel="所属档案门类流水号" />
					<aos:hiddenfield name="fieldmetch" fieldLabel="fieldmetch" value="0" />
					<aos:textfield name="tablename" fieldLabel="档案门类名称"
								   allowBlank="false" readOnly="true" />
					<aos:textfield name="tabledesc" fieldLabel="档案门类描述"
								   allowBlank="false" readOnly="true" />
			...
			<aos:window id="_w_field_u" title="修改档案门类字段">
				<aos:formpanel id="_f_field_u" width="500" layout="anchor"
							   labelWidth="90">
					<aos:hiddenfield name="id_" fieldLabel="档案门类字段流水号" />
					<aos:hiddenfield name="tid" fieldLabel="档案门类流水号" />
					<aos:textfield name="tablename" fieldLabel="档案门类名称"
								   allowBlank="false" readOnly="true" />
					<aos:textfield name="tabledesc" fieldLabel="档案门类描述"
								   allowBlank="false" readOnly="true" />
			...
			<aos:window id="_w_order"  title="显示顺序" height="530" autoScroll="true" onshow="_g_order_query">
				<aos:gridpanel hidePagebar="true" id="_g_order"
					...
					<aos:docked>
						<aos:dockeditem xtype="tbtext" text="档案门类字段" />
					</aos:docked>
					<aos:column type="rowno" />
					<aos:column header="流水号" dataIndex="id_" hidden="true" />
					<aos:column header="字段中文名称" dataIndex="fieldcnname" width="90" />
			...
			<aos:window id="_w_table" title="新增档案门类" height="500" autoScroll="true"  split="true">
				<aos:formpanel id="_f_table" width="800" layout="column"
							   labelWidth="75">
					<aos:textfield fieldLabel="档案门类名称" name="tablename"
								   allowBlank="false" vtype="alphanum" emptyText="只能输入字母和数字"
								   columnWidth="0.5" />
					<aos:textfield fieldLabel="档案门类描述" name="tabledesc"
								   allowBlank="false" columnWidth="0.49" />
					<aos:combobox fieldLabel="档案门类模板" name="tableTemplate"
			...
			<aos:window id="_w_field_demo" title="新增档案门类字段">
		...
		<script type="text/javascript">
			...
			function _g_table_reset(){
				...
				var msg=AOS.merge('档案门类内容将被清空且无法恢复！');
			...
			function _g_table_del() {
				...
				var msg = AOS.merge('确认要删除选中的[{0}]条档案门类吗？', rows);
				AOS.confirm(msg, function (btn) {
					if (btn === 'cancel') {
						AOS.tip('删除操作被取消。');
						return;
			...
			function _g_field_del(){
				...
				var msg = AOS.merge('确认要删除选中的[{0}]条档案门类字段吗？', rows);
				AOS.confirm(msg, function (btn) {
					if (btn === 'cancel') {
						AOS.tip('删除操作被取消。');
						return;
