	...
	<aos:onready>
		<aos:viewport layout="border">
			<aos:gridpanel id="_g_account" url="listAccounts2.jhtml" region="north" split="true" pageSize="${pagesize }"
						   enableLocking="true"  onrender="_g_account_query" onitemclick="path" rowclass="true">
				...
				<aos:column header="复审意见" dataIndex="fsyj" />
			...
			<aos:panel region="east" id="edit_message"  split="true" >
				...
				<aos:formpanel id="_f_description" title="鉴定" layout="column" autoScroll="true" width="600">
					...
					<aos:radioboxgroup fieldLabel="复审结果"  id="fushenjieguo" onshow="123"  columnWidth="0.49" columns="[100,100]" >
						<aos:radiobox name="c3" boxLabel="通过" onchang="onchang"/>
						<aos:radiobox name="c3" boxLabel="未通过" />
					</aos:radioboxgroup>
					<aos:textfield  id="fswtgly" name="fswtgly" fieldLabel="未通过理由" columnWidth="0.49"/>
				...
				<aos:docked dock="bottom" ui="footer">
					...
					<aos:dockeditem onclick="_next_save_kf_details" text="提交终审" icon="ok.png" />
				</aos:docked>
			</aos:panel>
			...
			<aos:window id="_w_next_kf_message" title="复核开放鉴定说明" width="500" >
				<aos:formpanel  id="_f_next_kf_message" width="480">
					<aos:textareafield fieldLabel="操作说明" id="next_kf_message"  name="next_kf_message" width="450" allowBlank="false"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem text="同意" onclick="_save_next_kf_message" icon="agree.png" />
					<aos:dockeditem text="拒绝" onclick="no_save_next_kf_message" icon="against.png" />
		...
		<script type="text/javascript">
			...
			function _save_write_compilation_message(){
				...
				AOS.ajax({
					...
					params:{
						...
						compilation_message:Ext.getCmp("next_kf_message").getValue(),
			...
			function booleanUpdate(){
				...
				var fsjg="";
				var fsCheck = Ext.getCmp('fushenjieguo').getRefItems();
				for(var i = 0; i < fsCheck.length; i++){
					if(fsCheck[i].checked){
						fsjg=fsCheck[i].boxLabel;
						break;
					}
				}
				//
				//复审结果
				if(fsjg!=grid.data.fsjg){
					return false;
				}

				//复审未通过理由
				if(form.findField("fswtgly").getValue()!=grid.data.fswtgly){
					return false;
				}
				//复审意见
				if(form.findField("fsyj").getValue()!=grid.data.fsyj){
					return false;
				}
				return true;
			}
			...
			function onchang(){
				...
				var scjg="";
				var scCheck = Ext.getCmp('fushenjieguo').getRefItems();
				for(var i = 0; i < scCheck.length; i++){
					if(scCheck[i].checked){
						scjg=scCheck[i].boxLabel;
						break;
				...
				if(scjg==null||scjg==""){
					Ext.getCmp("fswtgly").show();
				}else if(scjg=="未通过"){
					Ext.getCmp("fswtgly").show();
				}else{
					Ext.getCmp("fswtgly").hide();
				}
			};
			function radio_check(){
				...
				var fsCheck = Ext.getCmp('fushenjieguo').items;
				for(var i = 0; i < fsCheck.length; i++){
					if(data.fsjg!=null&&data.fsjg!=""){
						if(data.fsjg==fsCheck.get(i).boxLabel){
							fsCheck.get(i).setValue(true);
						}
					}
				}

			}
			function select_yes_fushen(){
				alert("点击了");
				Ext.getCmp("fswtgly").show();
			}
			function saveselect(){
...
//把这个值存放到数据库中
			...
			//保存开放信息
			function _next_save_kf_details(){
				//此时把当前的信息全部保存到数据表中并添加信息
				_w_next_kf_message.show();
				Ext.getCmp("next_kf_message").setValue("鉴定复审已完毕，请终审。");
			}
