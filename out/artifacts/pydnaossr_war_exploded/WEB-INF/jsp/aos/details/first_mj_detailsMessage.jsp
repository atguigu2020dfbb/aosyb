		...
		<aos:viewport layout="border">
			...
			<aos:panel region="east" id="edit_message"  split="true" >
				...
				<aos:docked dock="bottom" ui="footer">
					...
					<aos:dockeditem onclick="_first_save_mj_details" text="提交复审" icon="ok.png" />
				</aos:docked>
			</aos:panel>
			...
			<aos:window id="_w_first_mj_message" title="初鉴密级鉴定说明" width="500" >
				<aos:formpanel  id="_f_first_mj_message" width="480">
					<aos:textareafield fieldLabel="操作说明" id="first_mj_message"  name="first_mj_message" width="450" allowBlank="false"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem text="确定" onclick="_save_first_mj_message" icon="ok.png" />
					<aos:dockeditem onclick="#_w_first_mj_message.hide();" text="关闭"
									icon="close.png" />
				</aos:docked>
			</aos:window>
		</aos:viewport>
...
//字段信息传递过去
			...
			function path() {
				...
				/*for(var i=0;i<AOS.rows(_g_account);i++){
                    if(i==0){
...
//把这个值存放到数据库中
			...
			//保存开放信息
			function _first_save_mj_details(){
				//此时把当前的信息全部保存到数据表中并添加信息
				_w_first_mj_message.show();
				Ext.getCmp("first_mj_message").setValue("鉴定初审已完毕，请复审。");
			}
			function _save_first_mj_message(){
				//执行保存操作
				...
				AOS.ajax({
					url: 'savefirstmjdetails.jhtml',
					forms:_f_first_mj_message,
					params:{
