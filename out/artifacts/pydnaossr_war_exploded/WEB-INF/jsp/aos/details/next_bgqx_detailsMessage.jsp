	...
	<aos:onready>
		<aos:viewport layout="border">
			<aos:gridpanel id="_g_account" url="listAccounts2.jhtml" region="north" split="true" pageSize="${pagesize }"
				...
				<aos:menu>
					<aos:menuitem text="永久-30年" onclick="!save_jiazhi"   />
					<aos:menuitem text="30年-永久" onclick="!save_jiazhi" />
					<aos:menuitem text="永久-长期" onclick="!save_jiazhi"  />
					<aos:menuitem text="长期-永久" onclick="!save_jiazhi" />
					<aos:menuitem text="永久-定期" onclick="!save_jiazhi"  />
					<aos:menuitem text="定期-永久" onclick="!save_jiazhi"  />
				</aos:menu>
				<aos:column dataIndex="id_" header="流水号" hidden="true" />
				<aos:column dataIndex="_bgqx" header="价值类型" hidden="true" />
			...
			<aos:panel region="east" id="edit_message"  split="true" >
				<aos:formpanel id="_f_description" title="鉴定" layout="column" >
					...
					<aos:radioboxgroup fieldLabel="价值类型"  id="jiazhileixing" columns="[100,120,100]">
						<aos:radiobox name="_bgqx" boxLabel="永久-30年" />
						<aos:radiobox name="_bgqx" boxLabel="30年-永久" />
						<aos:radiobox name="_bgqx" boxLabel="永久-长期" />
						<aos:radiobox name="_bgqx" boxLabel="长期-永久" />
						<aos:radiobox name="_bgqx" boxLabel="永久-定期" />
						<aos:radiobox name="_bgqx" boxLabel="定期-永久" />
					</aos:radioboxgroup>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					...
					<aos:dockeditem onclick="_next_save_jiazhi_details" text="提交终审" icon="ok.png" />
				</aos:docked>
			</aos:panel>
			...
			<aos:window id="_w_next_jiazhi_message" title="再审价值鉴定说明" width="500" >
				<aos:formpanel  id="_f_next_jiazhi_message" width="480">
					<aos:textareafield fieldLabel="操作说明" id="next_jiazhi_message"  name="next_jiazhi_message" width="450" allowBlank="false"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem text="同意" onclick="_save_next_jiazhi_message" icon="agree.png" />
					<aos:dockeditem text="拒绝" onclick="no_save_next_jiazhi_message" icon="against.png" />
					<aos:dockeditem onclick="#_w_next_jiazhi_message.hide();" text="关闭"
									icon="close.png" />
				</aos:docked>
			</aos:window>
		</aos:viewport>
		<script type="text/javascript">
			...
			function _save_write_compilation_message(){
				...
				AOS.ajax({
					...
					params:{
						...
						compilation_message:Ext.getCmp("next_jiazhi_message").getValue(),
			...
			function radio_check(){
				var jiazhi=AOS.selectone(_g_account).raw._bgqx;
				var equRad=Ext.getCmp("jiazhileixing").getRefItems();
				for(var b=0;b<equRad.length;b++){
					if(equRad[b].boxLabel===jiazhi){
...
//把这个值存放到数据库中
			...
			//保存开放信息
			function _next_save_jiazhi_details(){
				//此时把当前的信息全部保存到数据表中并添加信息
				_w_next_jiazhi_message.show();
				Ext.getCmp("next_jiazhi_message").setValue("鉴定复审已完毕，请终审。");
			}
			function _save_next_jiazhi_message(){
				//执行保存操作
				...
				AOS.ajax({
					url: 'savenextbgqxdetails.jhtml',
					forms:_f_next_jiazhi_message,
					params:{
			...
			function no_save_next_jiazhi_message(){
				//执行保存操作
				//var tablename_gridData = JSON.stringify(Ext.pluck(_g_tablename_details_store.data.items, 'data'));
				var user="<%=session.getAttribute("user")%>";
				var _classtree=Ext.getCmp("_classtree").getValue();
