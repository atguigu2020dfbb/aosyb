...
<aos:html>
	...
	<aos:onready>
		<aos:viewport layout="fit" id="viewport">
			...
			<aos:gridpanel id="_g_data" url="listAccounts.jhtml"  region="sourth"
						   pageSize="100"   onitemclick="itemclick" rowclass="true" >
				<aos:docked>
					...
					<aos:dockeditem text="拟开放目录" icon="picture.png"  id="_f_filenames_message"
									onclick="dayinml"   />
				</aos:docked>
				<aos:selmodel type="checkbox" mode="multi" />
		...
		<script type="text/javascript">
			window.onload=function(){
				...
				Ext.getCmp("next_return").hide();
				Ext.getCmp("last_return").hide();
				...
				if(user=="root"){
					...
					Ext.getCmp("next_operator").show();
					Ext.getCmp("last_operator").show();
					Ext.getCmp("next_return").show();
					Ext.getCmp("last_return").show();
				}
			}
			...
			function get_a_topic(){
				var record = AOS.selectone1(_g_topic);
				var params = {
            ...
            function dayinml(){
			    var name="拟开放目录";
			    parent.fnaddtab('','打印目录','http://192.168.0.3/aosyb/ureport/preview?_u=file:'+name+'.ureport.xml&pid=12313');
			}
		</script>
	</aos:onready>
</aos:html>