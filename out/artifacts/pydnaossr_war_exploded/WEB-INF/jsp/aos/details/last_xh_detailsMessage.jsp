		...
		<aos:viewport layout="border">
			...
			<aos:panel region="east" id="edit_message"  split="true" >
				...
				<aos:docked dock="bottom" ui="footer">
					...
					<aos:dockeditem onclick="_last_save_xh_details" text="保存并添加描述" icon="ok.png" />
				</aos:docked>
			</aos:panel>
			...
			<aos:window id="_w_last_xh_message" title="终审销毁鉴定说明" width="500" >
				<aos:formpanel  id="_f_last_xh_message" width="480">
					<aos:textareafield fieldLabel="操作说明" id="last_xh_message"  name="last_xh_message" width="450" allowBlank="false"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
					<aos:dockeditem text="同意" onclick="_save_last_xh_message" icon="agree.png" />
					<aos:dockeditem text="拒绝" onclick="no_save_last_xh_message" icon="against.png" />
					<aos:dockeditem onclick="#_w_last_xh_message.hide();" text="关闭"
									icon="close.png" />
				</aos:docked>
			</aos:window>
		</aos:viewport>
...
//把这个值存放到数据库中
			...
			function _last_save_xh_details(){
				//此时把当前的信息全部保存到数据表中并添加信息
				_w_last_xh_message.show();
				Ext.getCmp("last_xh_message").setValue("");
			}
			function _save_last_xh_message(){
				//执行保存操作
				...
				AOS.ajax({
					url: 'savelastxhdetails.jhtml',
					forms:_f_last_xh_message,
					params:{
			...
			function no_save_last_xh_message(){
				//执行保存操作
				//var tablename_gridData = JSON.stringify(Ext.pluck(_g_tablename_details_store.data.items, 'data'));
				var user="<%=session.getAttribute("user")%>";
				var _classtree=Ext.getCmp("_classtree").getValue();
