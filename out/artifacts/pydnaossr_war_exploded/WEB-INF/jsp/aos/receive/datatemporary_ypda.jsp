...
<aos:html>
	...
	<aos:body>
		...
		<div id="filediv3" style="display: none;margin-top: 100px">
			...
			<p>
				<a style="text-align: center"><font color="red" style="margin-left: 20px">准确性</font></a>
			</p>
		</div>
	...
	<aos:onready>
		<aos:viewport layout="border" id="viewport">
			<aos:gridpanel id="_g_receive" url="listReceive_ypda.jhtml" height="250" region="north"   splitterBorder="1 0 1 0" split="true"
				...
				<aos:docked>
					...
					<aos:button text="导入" icon="folder8.png" scale="small" margin="0 0 0 0" id="importt">
						<aos:menu plain="false">
							...
							<aos:menuitem text="数据库导入" icon="folder8.png" onclick="_w_import_show" />
						</aos:menu>
					</aos:button>
					...
					<aos:dockeditem text="检测报告" icon="folder8.png" onclick="_w_sxjc_report" id="_i_sxjc_report" />
			...
			<aos:window id="_w_pici" title="新增任务" > <%--onshow="_new_task"--%>
				<aos:formpanel id="_f_pici" width="800" layout="column" labelWidth="90">
					...
					<aos:textfield name="njzjs" fieldLabel="总件数"  columnWidth="0.49"  />
					...
					<aos:combobox name="dzqwgs" value="MP3" fieldLabel="格式类型" labelWidth="90"  columnWidth="0.49" >
						<aos:option value="WAV" display="WAV" />
						<aos:option value="MP3" display="MP3" />
					</aos:combobox>
			...
			<aos:window id="_w_receive_u" title="修改"  >
				<aos:formpanel id="_f_receive_u" layout="column" width="800">
					...
					<aos:textfield name="dzqwrl" fieldLabel="容量大小"  columnWidth="0.49"  />
					<aos:textfield name="mlsjygs" fieldLabel="目录格式"  columnWidth="0.49"  />
					<aos:combobox name="dzqwgs" value="MP3" fieldLabel="格式类型" labelWidth="100"  columnWidth="0.49" >
						<aos:option value="WAV" display="WAV" />
						<aos:option value="MP3" display="MP3" />
					</aos:combobox>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				$("#file").fileinput({
					...
					allowedFileExtensions: ['xls','xlsx','jpg', 'gif', 'png','pdf','tif','dbf','mdb'],//接收的文件后缀
					showRemove: true,//显示删除按钮
					showUpload: true, //是否显示上传按钮
					showCaption: true,//是否显示标题
					browseClass: "btn btn-primary btn-xs", //按钮样式
			...
			function _new_task(){
				...
				if("jnws_temporary"==tablename){
					...
					lx="SP";
				}else if("ypda_temporary"==tablename){
					lx="YP";
				}
