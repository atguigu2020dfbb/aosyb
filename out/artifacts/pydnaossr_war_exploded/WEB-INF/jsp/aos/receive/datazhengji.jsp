...
<aos:html>
	...
	<aos:onready>
		<aos:viewport layout="border" id="viewport">
			<aos:gridpanel id="_g_archivezhengji" url="listAccounts_zhengji.jhtml" height="450" region="center" split="true"
						   onrender="_g_archivezhengji_query" pageSize="${pagesize }"  enableLocking="true">
				<aos:docked>
					...
					<aos:dockeditem text="新增任务" icon="add.png"   id="_f_add_message" onclick="_w_archivezhengji_show"/>
					<aos:dockeditem text="修改" icon="edit.png" onclick="_w_archivezhengji_u_show"  id="_f_editi_message" />
					<aos:dockeditem text="删除" icon="del2.png"  id="_f_delete_message"
									onclick="_g_archivezhengji_del"/>
			...
			<aos:window id="_w_archivezhengji" title="新增"   onshow="_new_task">
				<aos:formpanel id="_f_archivezhengji" layout="column" width="800">
					<aos:textfield name="zjbh" fieldLabel="征集编号" columnWidth="0.49" />
					<aos:datefield name="zjsj" fieldLabel="征集时间" columnWidth="0.49" value="${nd}"/>
					<aos:textfield name="zjbm" fieldLabel="征集部门" columnWidth="0.49"/>
					<aos:textfield name="zjr" fieldLabel="征集人" columnWidth="0.49" value="${user_en}"/>
					<aos:textfield name="yjbm" fieldLabel="移交部门" columnWidth="0.49"/>
					<aos:textfield name="yjr" fieldLabel="移交人" columnWidth="0.49"/>
					<aos:textfield name="tm" fieldLabel="题名" columnWidth="0.99"/>
					...
					<aos:textfield name="zhjsj" fieldLabel="转交时间"  columnWidth="0.49"/>
					...
					<aos:docked dock="bottom">
						<aos:dockeditem xtype="tbfill"/>
						<aos:dockeditem text="添加" icon="add.png" onclick="_f_archivezhengji_save"/>
						<aos:dockeditem text="关闭" icon="close.png" onclick="#_w_archivezhengji.hide()"/>
					</aos:docked>
				</aos:formpanel>
			</aos:window>
			<aos:window id="_w_archivezhengji_u" title="修改"  >
				<aos:formpanel id="_f_archivezhengji_u" layout="column" width="800">
					<aos:hiddenfield name="id_"  />
					<aos:textfield name="zjbh" fieldLabel="征集编号" columnWidth="0.49" />
					...
					<aos:textfield name="zjsj" fieldLabel="转交时间"  columnWidth="0.49"/>
					...
					<aos:docked dock="bottom">
						...
						<aos:dockeditem text="添加" icon="add.png" onclick="_f_archivezhengji_u_save"/>
						<aos:dockeditem text="关闭" icon="close.png" onclick="#_w_archivezhengji_u.hide()"/>
					</aos:docked>
				</aos:formpanel>
			</aos:window>
		</aos:viewport>
		<script type="text/javascript">
			function _f_archivezhengji_u_save(){
				var ztlx="";
				...
				//得到选项框选择的内容
				AOS.ajax({
					forms:_f_archivezhengji_u,
					params:{_classtree:cascode_id_.getValue(),ztlx:ztlx},
					url:'updateArchivezhengji.jhtml',
					ok:function(data){
						if(data.appcode===1){
							_w_archivezhengji_u.hide();
							_g_archivezhengji_store.reload();
							AOS.tip(data.appmsg);
						}else {
							AOS.err(AOS.appmsg);
						}
			...
			function _f_archivezhengji_save(){
				//得到选项框选择的内容
				var ztlx="";
				...
				AOS.ajax({
					forms:_f_archivezhengji,
					params:{_classtree:cascode_id_.getValue(),ztlx:ztlx},
					url:'saveArchivezhengji.jhtml',
					ok:function(data){
						if(data.appcode === 1){
							_w_archivezhengji.hide();
							_g_archivezhengji_store.reload();
							AOS.tip(data.appmsg);
						}else {
							AOS.err(data.appmsg);
			...
			function _w_archivezhengji_u_show(){
				var record = AOS.selectone(_g_archivezhengji);
				if(record) {
					_w_archivezhengji_u.show();
					_f_archivezhengji_u.loadRecord(record);
					var xqCheck = Ext.getCmp('ztlx_update').items;
					for(var i = 0; i < xqCheck.length; i++){
			...
			function _new_task(){
				//根据tree名称得到对应的部门名称
				AOS.ajax({
					params:{name_:"档案征集流水号",
					tablename:tablename.getValue(),
					zjnd:Ext.getCmp("nd").getValue()
					},
					url:'dazjlsh.jhtml',
					ok:function(data){
						_f_archivezhengji.form.findField("zjbh").setValue("DZ"+Ext.getCmp("nd").getValue()+data.index);
					}
				});
			}
			function _w_archivezhengji_show(){
				_w_archivezhengji.show();
			}
			//加载数据,（批次号列表信息）
			...
			//加载表格数据
			function _g_archivezhengji_query() {
				var params = {
					tablename : tablename.getValue(),
					zjsj:Ext.getCmp("nd").getValue(),
					cascode_id_:cascode_id_.getValue()
				};
				//这个Store的命名规则为：表格ID+"_store"。
				_g_archivezhengji_store.getProxy().extraParams = params;
				_g_archivezhengji_store.load();
			}
			//父表点击，字表加载
			function get_a_data(){
				var record = AOS.selectone(_g_archivezhengji);
				...
				//这个Store的命名规则为：表格ID+"_store"。
				_g_archivezhengji_from_store.getProxy().extraParams = params;
				_g_archivezhengji_from_store.load();
			}
			...
			function fn_g_archivezhengji(){
				var selection = AOS.selection(_g_archivezhengji, 'id_');
				var tms = AOS.selection(_g_archivezhengji, 'tm');
				if (AOS.empty(selection)) {
					AOS.tip('修改前请先选中数据。');
					return;
				}
				AOS.ajax({
					params: {id_: AOS.selectone1(_g_archivezhengji).data.id_,
						tablename:tablename.getValue()
					},
					url: 'getData.jhtml',
					ok: function (data) {
			...
			function fn_g_archivezhengji_from(){
				var selection = AOS.selection(_g_archivezhengji_from, 'id_');
				var tms = AOS.selection(_g_archivezhengji_from, 'tm');
				if (AOS.empty(selection)) {
					AOS.tip('修改前请先选中数据。');
					return;
				}
			...
			function _f_data_query(){
				...
				params["zjsj"]=Ext.getCmp("nd").getValue();
				_g_archivezhengji_store.getProxy().extraParams = params;
				_g_archivezhengji_store.load();
				_w_query_q.hide();
				AOS.reset(_f_query);
			...
			function _f_data_replace_from(){
				...
				AOS.ajax({
					...
					ok : function(data) {
						_w_query_q_from.hide();
						_g_archivezhengji_from_store.reload();
						AOS.tip(data.appmsg);
					}
				});
			}
			...
			function _w_data_i_from_onshow(){
				if(typeof(AOS.selectone(_g_archivezhengji_from))!='undefined'){
					AOS.ajax({
						params: {id_: AOS.selectone(_g_archivezhengji_from).data.id_,
			...
			function _f_data_edit_from(){
				var record_from = AOS.selectone(_g_archivezhengji_from);
				var record = AOS.selectone(_g_archivezhengji);
				AOS.ajax({
					forms : _f_data_i_from,
			...
			//删除信息
			function _g_archivezhengji_del() {
				var selection = AOS.selection(_g_archivezhengji, 'id_');
				if (AOS.empty(selection)) {
					AOS.tip('删除前请先选中数据。');
					return;
				}
				var msg = AOS.merge('确认要删除选中的[{0}]个任务数据吗？', AOS.rows(_g_archivezhengji));
				AOS.confirm(msg, function (btn) {
					if (btn === 'cancel') {
						AOS.tip('删除操作被取消。');
						return;
					...
					AOS.ajax({
						url: 'deleteArchivezhengji.jhtml',
						params: {
							aos_rows_: selection,
							tablename: tablename.getValue()
						},
			...
			//删除信息
			function _g_archivezhengji_from_del() {
				var selection = AOS.selection(_g_archivezhengji_from, 'id_');
				var tms = AOS.selection(_g_archivezhengji_from, 'tm');
				if (AOS.empty(selection)) {
					AOS.tip('删除前请先选中数据。');
				...
				var msg = AOS.merge('确认要删除选中的[{0}]个用户数据吗？', AOS.rows(_g_archivezhengji_from));
				AOS.confirm(msg, function (btn) {
					if (btn === 'cancel') {
						AOS.tip('删除操作被取消。');
						return;
			...
			function w_data_path_onshow() {
				...
				var record = AOS.selectone(Ext.getCmp('_g_archivezhengji'));
				var me=Ext.getCmp("uploadpanel");
				me.store.removeAll();
				AOS.ajax({
					params: {tid: record.data.id_,tablename:tablename.getValue()},
			...
			function _g_receive_pici(){
				...
				AOS.confirm(msg, function (btn) {
					...
					AOS.ajax({
						params:{'id_':id_,aos_module_id_:aos_module_id_,
							tablename:tablename,
			...
			function _w_test_onshow(){
				...
				var record = AOS.selectone(_g_archivezhengji,true);
			...
			function _f_previous_data(){
				var count=Ext.getCmp("_g_archivezhengji").getStore().getCount();
				var me=Ext.getCmp("_g_archivezhengji").getSelectionModel().getSelection();
				//var record = AOS.selectone(_g_archivezhengji);
				//得到执行行的坐标
				var rowIndex = Ext.getCmp("_g_archivezhengji").getStore().indexOf(me[0]);
				if(rowIndex==0){
					AOS.err("当前第一条!");
					return;
				}
				var s=Ext.getCmp("_g_archivezhengji").getStore().getAt(rowIndex-1);
				//原先行取消选中
				Ext.getCmp("_g_archivezhengji").getSelectionModel().deselect(rowIndex);
				//此时让光标选中上一行
				Ext.getCmp("_g_archivezhengji").getSelectionModel().select(rowIndex-1, true);
				//组件被显示后触发。
				Ext.getCmp("_f_archivezhengji_u").form.setValues(s.data);
			}
			//下一页
	...
	<script type="text/javascript">
		function _w_details_show(a){
			var record=Ext.getCmp('_g_archivezhengji').getSelectionModel().getSelection();
		...
		function w_data_path_onclose(){
			Ext.getCmp("_g_archivezhengji").getStore().load();
