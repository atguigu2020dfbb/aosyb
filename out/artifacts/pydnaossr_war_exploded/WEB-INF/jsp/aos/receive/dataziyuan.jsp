...
<aos:html>
	...
	<aos:onready>
		<aos:viewport layout="border" id="viewport">
			<aos:gridpanel id="_g_ziyuan" url="listziyuan.jhtml" height="250" region="north" split="true"
						   onrender="_g_ziyuan_query" pageSize="${pagesize }"  onitemclick="get_a_receive" enableLocking="true">
				<aos:docked>
					...
					<aos:hiddenfield id="jsnd" name="jsnd" value="${jsnd}"/>
					<aos:dockeditem xtype="tbtext" text="接收信息" />
					<aos:dockeditem text="新增任务" icon="add.png"   onclick="_w_ziyuan_show"/>
					<aos:dockeditem text="修改" icon="edit.png" onclick="_w_ziyuan_u_show"  id="_f_editi_message" />
					<aos:dockeditem text="删除" icon="del2.png" onclick="_g_ziyuan_del"/>
				...
				<aos:column dataIndex="jsbh" header="接收编号"  width="100" />
				<aos:column dataIndex="nd" header="接收年度"  width="100" />
				<aos:column dataIndex="yjbm" header="移交部门"  width="100" />
				<aos:column dataIndex="yjr" header="移交人"  width="100" />
				<aos:column dataIndex="jjsj" header="交接时间"  width="100" />
				<aos:column dataIndex="jsr" header="接收人"  width="100" />
				<aos:column dataIndex="tm" header="题名"  width="100" />
				<aos:column dataIndex="xcdw" header="形成单位"  width="100" />
				...
				<aos:column dataIndex="wjdx" header="文件大小"  width="100" />
				<aos:column dataIndex="sfzj" header="是否转交"  width="100" />
				<aos:column dataIndex="zjsj" header="转交时间"  width="100" />
				<aos:column dataIndex="zjbm" header="转交部门"  width="100" />
				<aos:column dataIndex="zhjr" header="转交人"  width="100" />
				<aos:column dataIndex="bz" header="备注"  width="100" />
				<aos:column dataIndex="yjssc" header="移交书上传"  width="150" />
			...
			<aos:window id="_w_ziyuan" title="新增"   onshow="_new_task">
				<aos:formpanel id="_f_ziyuan" layout="column" width="800">
					<aos:textfield name="jsbh" fieldLabel="接收编号" columnWidth="0.49" />
					<aos:textfield name="nd" fieldLabel="接收年度" value="${jsnd}" columnWidth="0.49"/>
					...
					<aos:textfield name="xcsj" fieldLabel="形成时间" columnWidth="0.49"/>
					<aos:checkboxgroup fieldLabel="载体类型" columnWidth="0.49" id="ztlx_add" columns="[80,80]">
						<aos:checkbox name="c5" boxLabel="图片" />
						<aos:checkbox name="c5" boxLabel="视频" />
					</aos:checkboxgroup>
					<aos:textfield name="ys" fieldLabel="页数" columnWidth="0.49"/>
					<aos:textfield name="sc" fieldLabel="时长" columnWidth="0.49"/>
					<aos:textfield name="wjdx" fieldLabel="文件大小"  columnWidth="0.49"/>

					<aos:textfield name="bz" fieldLabel="备注" columnWidth="0.98"/>
					<aos:hiddenfield id="cjr" name="cjr"  value="${user}"/>
					<aos:hiddenfield id="lx" name="lx" value="${lx}"/>
					<aos:docked dock="bottom">
						<aos:dockeditem xtype="tbfill"/>
						<aos:dockeditem text="添加" icon="add.png" onclick="_f_ziyuan_save"/>
						<aos:dockeditem text="关闭" icon="close.png" onclick="#_w_ziyuan.hide()"/>
					</aos:docked>
				</aos:formpanel>
			</aos:window>
			<aos:window id="_w_ziyuan_u" title="修改"  >
				<aos:formpanel id="_f_ziyuan_u" layout="column" width="800">
					...
					<aos:textfield name="xcdw" fieldLabel="形成单位" columnWidth="0.49"/>
					<aos:textfield name="xcsj" fieldLabel="形成时间" columnWidth="0.49"/>
					<aos:checkboxgroup fieldLabel="载体类型" columnWidth="0.49" id="ztlx_update" columns="[80,80]">
						<aos:checkbox name="c5" boxLabel="图片" />
						<aos:checkbox name="c5" boxLabel="视频" />
					</aos:checkboxgroup>
					<aos:textfield name="ys" fieldLabel="页数" columnWidth="0.49"/>
					...
					<aos:docked dock="bottom">
						...
						<aos:dockeditem text="保存" icon="save.png" onclick="_f_ziyuan_u_save"/>
						<aos:dockeditem text="关闭" icon="close.png" onclick="#_w_ziyuan_u.hide()"/>
					</aos:docked>
				</aos:formpanel>
			</aos:window>
			<aos:window id="_w_data_i" title="新增" width="750" >
				<aos:formpanel id="_f_data_i" layout="column"   width="750"  >
					<aos:hiddenfield  name="_classtree" id="_classtree" value="${cascode_id_}" />
					<aos:textfield name="nd" fieldLabel="年度" value="${jsnd}" columnWidth="0.49"/>
					...
					<%--<aos:hiddenfield id="cjr" name="cjr" />--%>
					<aos:hiddenfield id="lx" name="lx" value="${lx}"/>
				</aos:formpanel>
				<aos:docked dock="bottom" ui="footer">
					<aos:dockeditem xtype="tbfill" />
		...
		<script type="text/javascript">
			...
			function _w_data_show() {
				var record = AOS.selectone(_g_ziyuan);
				if(record) {
					AOS.reset(_f_data_i);
					_w_data_i.show();
			...
			function _f_ziyuan_u_save(){
				...
				AOS.ajax({
					forms:_f_ziyuan_u,
					params:{_classtree:cascode_id_.getValue(),ztlx:ztlx},
					url:'updateZiyuan.jhtml',
					ok:function(data){
						if(data.appcode===1){
							_w_ziyuan_u.hide();
							_g_ziyuan_store.reload();
							var params = {
			...
			function _w_ziyuan_u_show(){
				var record = AOS.selectone(_g_ziyuan);
				if(record) {
					_w_ziyuan_u.show();
					_f_ziyuan_u.loadRecord(record);
					var xqCheck = Ext.getCmp('ztlx_update').items;
					for(var i = 0; i < xqCheck.length; i++){
			...
			function _new_task(){
				...
				AOS.ajax({
					params:{name_:"资源接收流水号",
						tablename:tablename,
						zynd:Ext.getCmp("nd").getValue()
					},
					url:'zyjslsh.jhtml',
					ok:function(data){
						if("ybcb_receive"==tablename){
							_f_ziyuan.form.findField("jsbh").setValue("CB"+Ext.getCmp("nd").getValue()+data.index);
						}else if("ybrb_receive"==tablename){
							_f_ziyuan.form.findField("jsbh").setValue("RB"+Ext.getCmp("nd").getValue()+data.index);
						}else if("ybxw_receive"==tablename){
							_f_ziyuan.form.findField("jsbh").setValue("XW"+Ext.getCmp("nd").getValue()+data.index);
						}

					}
				});
				_f_ziyuan.form.findField("cjr").setValue("<%=session.getAttribute("user")%>");
			}
			function fn_g_data(){
				...
				if(record) {
					_w_data_i.show();
					_f_data_i.loadRecord(record);


					var ztCheck = Ext.getCmp('zztlx_add').items;
					for(var i = 0; i < ztCheck.length; i++){
						if(record.data.ztlx.indexOf(ztCheck.get(i).boxLabel)!=-1){
			...
			function _g_receive_pici(){
				var selection = AOS.selection(_g_ziyuan, 'id_');
				if (AOS.empty(selection)) {
					AOS.tip('提交前请先选中数据。');
					return;
				}
				...
				var msg = AOS.merge('确认要提交选中的[{0}]个任务数据吗？', AOS.rows(_g_ziyuan));
			...
			//加载表格数据
			function _g_ziyuan_query() {
				var params = {
					lx:${lx},
					jsnd:${jsnd}
				};
				//这个Store的命名规则为：表格ID+"_store"。
				_g_ziyuan_store.getProxy().extraParams = params;
				_g_ziyuan_store.load();
				var params = {
					tablename : tablename.getValue(),
					pch:"没有数据"
				};
			...
			function _f_data_i_save(){
				...
				AOS.ajax({
					forms : _f_data_i,
					url : 'saveData_ziyuan.jhtml',
					params:{
						tablename : tablename.getValue(),
						ztlx:ztlx,
						pch:AOS.selectone(_g_ziyuan).data.jsbh,
			...
			function _f_data_edit(){
				...
				AOS.ajax({
					forms : _f_data_i,
					url : 'updateData_ziyuan.jhtml',
					params:{
						tablename : tablename.getValue(),
						ztlx:ztlx,
						pch:record.data.jsbh,
						id_:archive_record.data.id_
					},
					ok : function(data) {
						if (data.appcode === -1) {
							AOS.err(data.appmsg);
			...
			//删除信息
			function _g_ziyuan_del() {
				var selection = AOS.selection(_g_ziyuan, 'id_');
				if (AOS.empty(selection)) {
					AOS.tip('删除前请先选中数据。');
					return;
				}
				var msg = AOS.merge('确认要删除选中的[{0}]个任务数据吗？', AOS.rows(_g_ziyuan));
				AOS.confirm(msg, function (btn) {
					if (btn === 'cancel') {
						AOS.tip('删除操作被取消。');
						return;
			...
			function open_file(){
				...
				var pch = AOS.selectone(_g_ziyuan).raw.jsbh;
				AOS.ajax({
					params : {
						pch:pch
					},
					url : 'booleanfile_ziyuan.jhtml',
					ok : function(data) {
						if(data.appcode===-1){
							AOS.tip("附件不存在");
							return;
            ...
            function _fileupload_show() {
                ...
                $(document).ready(function() {
                    $("#file").fileinput({
                        language: 'zh', //设置语言
                        uploadUrl: '${cxt}/archive/upload/uploadTemporary_ziyuan.jhtml', //上传的地址
                        uploadExtraData:{
                            tablename: tablename,
                            id_:AOS.selectone(_g_ziyuan).data.id_
                        },
                        fileActionSettings:{
			...
			function _w_import_show() {
				...
				$(document).ready(function() {
					$("#file").fileinput({
						...
						allowedFileExtensions: ['xls','xlsx','jpg'],//接收的文件后缀
						showRemove: true,//显示删除按钮
						showUpload: true, //是否显示上传按钮
						showCaption: true,//是否显示标题
						browseClass: "btn btn-primary btn-xs", //按钮样式
			...
			function _w_formal_receive(){
				...
				var msg = AOS.merge('确认要提交[{0}]个批次数据吗？', AOS.rows(_g_ziyuan));
				AOS.confirm(msg, function (btn) {
					if (btn === 'cancel') {
						AOS.tip('提交操作被取消。');
						return;
					...
					if(flag_submit=="已提交"&&flag_examine=="已通过"){
						...
						AOS.ajax({
							params:{id_:id_,tablename:tablename,pch:pch,nd:nd,rw_tablename:"zyjs_n",lx_bh:"jsbh"},
			...
			function _f_previous_data(){
				var count=Ext.getCmp("_g_ziyuan").getStore().getCount();
				var me=Ext.getCmp("_g_ziyuan").getSelectionModel().getSelection();
				//var record = AOS.selectone(_g_ziyuan);
				//得到执行行的坐标
				var rowIndex = Ext.getCmp("_g_ziyuan").getStore().indexOf(me[0]);
				if(rowIndex==0){
					AOS.err("当前第一条!");
					return;
				}
				var s=Ext.getCmp("_g_ziyuan").getStore().getAt(rowIndex-1);
				//原先行取消选中
				Ext.getCmp("_g_ziyuan").getSelectionModel().deselect(rowIndex);
				//此时让光标选中上一行
				Ext.getCmp("_g_ziyuan").getSelectionModel().select(rowIndex-1, true);
				//组件被显示后触发。
				Ext.getCmp("_f_ziyuan_u").form.setValues(s.data);
			}
			//下一页
