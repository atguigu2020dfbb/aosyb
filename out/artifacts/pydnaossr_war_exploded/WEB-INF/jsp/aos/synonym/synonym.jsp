...
<aos:html>
	<aos:head title="同义词定义">
		<aos:include lib="ext" />
		<aos:base href="synonym/synonym"/>
	</aos:head>
	<aos:body>
	</aos:body>
	<aos:onready><!-- -->
		<aos:viewport layout="fit">
			<aos:gridpanel id="_g_synonym" url="listsynonym.jhtml"
						   onrender="_g_synonym_query">
				<aos:docked>
					<aos:dockeditem xtype="tbtext" text="同义词定义表" />
					<aos:dockeditem xtype="tbseparator" />
					<aos:dockeditem text="新增" icon="folder15.png" onclick="_w_synonym_add" />
					<aos:dockeditem text="修改" icon="folder15.png" onclick="_w_synonym_update" />
					<aos:dockeditem text="删除" icon="del.png" onclick="_w_synonym_del" />
				</aos:docked>
				<aos:column type="rowno" />
				<aos:column header="流水号" dataIndex="id_" hidden="true" align="center" />
				<aos:column header="同义词编号" dataIndex="bh_number" width="60"
							align="center" />
				<aos:column header="同义词名称" dataIndex="name_" width="60"
							align="center" />
				<aos:column header="同义词描述" dataIndex="desc_" width="60"
							align="center" />
				<aos:column header="同义词列表" dataIndex="synonym_list" width="60"
							align="center" />
			...
			<aos:window id="_w_add" title="新增">
				<aos:formpanel id="_f_add" width="400" layout="column"
					...
					<aos:textfield name="name_" fieldLabel="同义词名称" columnWidth="0.99"/>
					<aos:textfield name="desc_" fieldLabel="描述" columnWidth="0.99"/>
					<aos:textfield name="synonym" fieldLabel="同义词填写" columnWidth="0.8"/>
					<aos:button text="添加" onclick="add_synonym" margin="0 0 0 10"  columnWidth="0.2"/>
			...
			<aos:window id="_w_update" title="修改" onshow="_w_update_show">
				<aos:formpanel id="_f_update" width="400" layout="column"
					...
					<aos:textfield name="desc_" fieldLabel="描述" columnWidth="0.99"/>
					<aos:textfield name="synonym" fieldLabel="同义词填写" columnWidth="0.8"/>
					<aos:button text="添加" onclick="add_synonym2" margin="0 0 0 10"  columnWidth="0.2"/>
		...
		<script type="text/javascript">
			...
			function add_synonym(){
				var fieldcnname=_f_add.getForm().findField('synonym').getValue();
				var params = [{
					'fieldcnnames':fieldcnname
				}];
				var count=_g_field_store.getCount();
			...
			function add_synonym2(){
				var fieldcnname=_f_update.getForm().findField('synonym').getValue();
				var params = [{
					'fieldcnnames':fieldcnname
				}];
			...
			//加载列表
			function _g_synonym_query(){
				_g_synonym_store.reload();
			}
			function _w_synonym_add(){
				_w_add.show();
			...
			function _w_synonym_update(){
				var record=AOS.selectone(_g_synonym);
				if(AOS.empty(record)){
					AOS.err("请选择要修改的全宗信息");
					return;
				}
			...
			function _w_update_show(){
				...
				var fieldcnnames=record.raw.synonym_list;
			...
			function _w_synonym_del(){
				var record = AOS.selectone(_g_synonym, true);
				var msg = AOS.merge('确认要删除选中的[{0}]个统计定义吗？', AOS.rows(_g_synonym));
				if (AOS.empty(record)) {
					AOS.tip('删除前请先选中数据!');
					return;
				}else{
