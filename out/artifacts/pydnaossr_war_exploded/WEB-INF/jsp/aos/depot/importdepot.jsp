...
<aos:head title="数据导入">
	...
	<aos:base href="archive/importTemporary" />
</aos:head>
<aos:body>
</aos:body>
<aos:onready>
  	<aos:viewport layout="fit">
		<aos:gridpanel id="_g_excel" url="getExcel.jhtml" hidePagebar="true" onrender="_g_excel_account" >
			...
			<aos:hiddenfield id="tablename" value="${tablename }"/>
				<aos:hiddenfield id="pch" value="${pch }"/>
	...
	swfu = new SWFUpload({
	upload_url : "${cxt}/archive/upload/uploadExcel.jhtml?kk2=kk2", //接收上传的服务端url
	...
	window.location.href="loadExcelGrid.jhtml?localFilename="+localFilename.getValue()+"&_classtree="+_classtree.getValue()+"&pch="+pch.getValue();
	}
	...
	function _select_field(){
		var _grid = Ext.getCmp('_g_import');
		var record = _g_import.selModel.getSelected(); // 获取当前行
		record.data.fieldenname = 'xxx'; // 这里给code和name两列赋上指定值
	}
	//确定导入
	function fn_ok(){
