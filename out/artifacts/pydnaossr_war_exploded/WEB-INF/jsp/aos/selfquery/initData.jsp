    ...
    <aos:onready>
        <aos:viewport layout="border">
            <aos:gridpanel id="_g_data" url="zz_listAccounts.jhtml" region="center" split="true"
                           onrender="_g_data_query" pageSize="50"
                           enableLocking="false" >
                <aos:docked>
                    ...
                    <aos:hiddenfield id="formid" name="formid" />
                    <!-- 存储当前页面的查询条件 -->
                    ...
                    <aos:dockeditem text="提交申请" icon="folder23.png"  onclick="_w_tjsq_show" />
        ...
        <script type="text/javascript">
            function select_archive(){
                var xm = Ext.getCmp("_f_apply").getForm().findField('xm').getValue();
                AOS.ajax({
                    forms : _f_apply,
                    url : 'addSFXX.jhtml',
                    ...
                    ok : function(data) {
                        //alert(data.id);
                        Ext.getCmp("formid").setValue(data.id);
                        if (data.appcode === -1) {
                            AOS.err(data.appmsg);
                        } else {
                            params={
            ...
            function _w_readCard_show(){
                _w_readCard.show();
                {
                    ...
                    {
                        Ext.getCmp("xm").setValue(jsValue.resultContent.partyName);
                        if(jsValue.resultContent.gender=="1"){
                            Ext.getCmp("xb").setValue("男");
                        }else if(jsValue.resultContent.gender=="0"){
                            Ext.getCmp("xb").setValue("女");
                        }

                        Ext.getCmp("mzgj").setValue(jsValue.resultContent.nation);
                        Ext.getCmp("cs").setValue(jsValue.resultContent.bornDay);
                        Ext.getCmp("dz").setValue(jsValue.resultContent.certAddress);
                        Ext.getCmp("sfzh").setValue(jsValue.resultContent.certNumber);
                        Ext.getCmp("qfjg").setValue(jsValue.resultContent.certOrg);
                        Ext.getCmp("tpsj").setValue(jsValue.resultContent.identityPic);
                        ...
                        //alert(jsValue.resultContent.identityPic);
            ...
            function _w_tjsq_show(){
                ...
                AOS.ajax({
                    url:"TjsqConsult.jhtml",
                    params:{
                        zzbh:Ext.getCmp("zzbh").getValue(),
                        tablename:tablename,
                        tablename_id:selection.data.id_,
                        xm:Ext.getCmp("xm").getValue(),
                        xb:Ext.getCmp("xb").getValue(),
                        sfzh:Ext.getCmp("sfzh").getValue(),
                        zzbh:Ext.getCmp("zzbh").getValue(),
                        formid : formid.getValue(),
                        tid:selection.data.id_,
                        state:2
                    },
                    ok:function(data){
                        AOS.tip(data.appmsg);
